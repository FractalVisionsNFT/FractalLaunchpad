...
contract FractalERC1155UpgradeableTest is Test {
    ...
    function setUp() public {
        ...
        launchpad = new FractalLaunchpad(
            feeRecipient, 0, address(implementation), address(erc721Implementation), address(factory)
        );
        ...
        proxy = FractalERC1155Impl(config.tokenContract);
    }
    ...
    function test_Upgrade_WithInitialization() public {
        ...
        bytes memory upgradeData = abi.encodeWithSelector(FractalERC1155ImplV2.initializeV2.selector, 99999);
    ...
    function test_StorageLayout_PreservationAcrossUpgrades() public {
        ...
        uint256[4] memory totalSupplies =
            [proxy.totalSupply(0), proxy.totalSupply(1), proxy.totalSupply(2), proxy.totalSupply(3)];

        uint256[4] memory maxSupplies = [proxy.maxSupply(0), proxy.maxSupply(1), proxy.maxSupply(2), proxy.maxSupply(3)];

        string[4] memory tokenURIs = [proxy.uri(0), proxy.uri(1), proxy.uri(2), proxy.uri(3)];
    ...
    function test_MultipleUpgrades_Success() public {
        ...
        UUPSUpgradeable(address(proxy)).upgradeToAndCall(
            address(newImplementationV2), abi.encodeWithSelector(FractalERC1155ImplV2.initializeV2.selector, 100)
        );
        vm.stopPrank();
    ...
    function testFuzz_Upgrade_PreservesTokenBalances(uint8 numTokens, uint8 numUsers) public {
        ...
        for (uint256 tokenId = 0; tokenId < numTokens; tokenId++) {
            for (uint256 userIdx = 0; userIdx < numUsers; userIdx++) {
                assertEq(upgradedProxy.balanceOf(users[userIdx], tokenId), balances[userIdx][tokenId]);
            }
        }
    }
